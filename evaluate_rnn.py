...
def visualize_inputs(ff,ff_type,bi,start_ff):
    size=ff.shape[1]
    if ff_type == 'magnitude':
        ...
        for true_ff in ff:
            ...
            if bi:
                ...
                plt.subplot(3,3,i)
            if ff_type=='velocity':
                plt.quiver(x,y,u1,v1, scale=140)
            else:
                plt.quiver(x,y,u1,v1, scale=20)
            i += 1
        if bi:
            plt.suptitle('%d CAD to %d CAD, step=2 CAD' % (-300 + start_ff*2, -300 + (start_ff+9)*2))
        else:
            plt.suptitle('%d CAD to %d CAD, step=2 CAD' % (-300 + start_ff*2, -300 + (start_ff+8)*2))
        plt.show()
...
def visualize_seq(ff_type, ff, bi, start_ff):
    if ff_type == 'magnitude':
        if bi:
            visualize_inputs(ff[:, 8:17, 8:17], ff_type, bi, start_ff)
        else:
            visualize_inputs(ff[:9, 8:17, 8:17], ff_type, bi, start_ff)
    else:
        if bi:
            visualize_inputs(ff[:, 8:17, 8:17, :], ff_type, bi, start_ff)
        else:
...
def read_inputs(ff_type, a, start_ff, bi):
    ...
    if ff_type=='direction':
        if bi:
            vf = np.array(utils.read_flow_field(os.path.join('data/velocity', str(a+start_ff*100+500))))
            true_v = vf[8:17, 8:17, :]
        else:
            vf = np.array(utils.read_flow_field(os.path.join('data/velocity', str(a+start_ff*100+900))))
            true_v = vf[8:17, 8:17, :]
    return ff, true_ff, true_v
...
def main(ff_type, input_size, bi):
    ...
    distance = np.linalg.norm(pred-true_ff.reshape(isq,-1), axis=1).reshape(1,-1)
